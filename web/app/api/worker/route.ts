...
function resolveSelections(sceneGraph: SceneGraph, selections: Record<string, string>) {
...
function buildGenerationPrompt(sceneGraph: SceneGraph, selectionLines: string[]) {
  const lockNotes = [
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { lines } = resolveSelections(sceneGraph, job.selections || {});
    const generationPrompt = buildGenerationPrompt(sceneGraph, lines);
