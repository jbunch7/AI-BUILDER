...
function extrasToLines(extras: Record<string, boolean> = {}) {
  const lines: string[] = [];
  if (extras.appliances) lines.push("- Appliances: update to modern cohesive finishes (stainless or matte black), same positions");
  if (extras.lighting) lines.push("- Lighting: upgrade fixtures (warm modern style), keep ceiling height and locations");
  if (extras.hardware) lines.push("- Hardware: update knobs/handles/faucets to a modern cohesive style");
  if (extras.backsplash) lines.push("- Backsplash: add or update to a clean modern backsplash (if applicable)");
  if (extras.countertop) lines.push("- Countertops: modernize surface material (if applicable)");
  if (extras.landscaping) lines.push("- Landscaping: tidy and enhance lawn/planting beds, keep hardscape layout");
  if (extras.garden) lines.push("- Garden: add tasteful garden beds/planters that fit the existing layout");
  if (extras.decor) lines.push("- Decor: add subtle staging/decor that matches the style without clutter");
  return lines;
}

function buildGenerationPrompt(sceneGraph: SceneGraph, selectionLines: string[], extraLines: string[], userPrompt?: string) {
  const lockNotes = [
...
OPTIONAL UPGRADES (IF REQUESTED):
${extraLines.join("\n") || "- (none)"}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const extraLines = extrasToLines(job.extras || {});
    const generationPrompt = buildGenerationPrompt(sceneGraph, lines, extraLines, job.userPrompt);
