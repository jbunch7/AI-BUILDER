...
export default function Page() {
  ...
  async function onPickFile(f: File) {
    ...
    setModules([]);
    setSelections({});
    setExtras({ appliances: false, lighting: false, backsplash: false, countertop: false, hardware: false, landscaping: false });
    setUserPrompt("");
  ...
  async function runScan() {
    ...
    try {
      ...
      const res2 = await fetch("/api/options", {
        ...
        body: JSON.stringify({ scanId: data.scanId, extras }),
      });
  ...
  // When feature toggles change, refresh available modules/options
  useEffect(() => {
    if (!scanId) return;
    let cancelled = false;
    (async () => {
      try {
        const res = await fetch("/api/options", {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ scanId, extras }),
        });
        const data = await res.json().catch(() => ({}));
        if (!res.ok) return;
        if (cancelled) return;
        ...
        setSelections((prev) => {
          const next = { ...prev };
          // Ensure every module has a selected option (keep prior if still valid)
          for (const m of (data.modules || []) as any[]) {
            const chosen = next[m.featureId];
            const stillValid = m.options?.some((o: any) => o.id === chosen);
            if (!stillValid) {
              if (m.defaultOptionId) next[m.featureId] = m.defaultOptionId;
              else if (m.options?.[0]) next[m.featureId] = m.options[0].id;
            }
          }
          return next;
        });
  ...
  async function runRender() {
    ...
    try {
      const res = await fetch("/api/render", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ scanId, selections, extras, userPrompt }),
      });
      const data = await res.json().catch(() => ({}));
    ...
    const draw = () => {
      ...
      for (const m of modules) {
        ...
        for (const el of elems) {
          ...
          if (isColor) {
            ctx.fillStyle = preview.hex;
            ctx.fillRect(0, 0, rect.width, rect.height);
          } else if (isImage) {
            const imgEl = textureCache.get(preview.src) || new Image();
            if (!textureCache.has(preview.src)) {
              imgEl.crossOrigin = "anonymous";
              imgEl.src = preview.src;
              imgEl.onload = () => draw();
              textureCache.set(preview.src, imgEl);
            }
            ...
            >
              ...
              >
                ...
                <div style={{ opacity: 0.8, marginTop: 6, fontSize: 13 }}>
                  Click swatches to preview instantly on the photo. Use “Render photoreal” for a realistic final.
                </div>

                <div style={{ marginTop: 14, padding: 12, borderRadius: 14, border: "1px solid rgba(255,255,255,0.10)", background: "rgba(255,255,255,0.04)" }}>
                  <div style={{ fontWeight: 800, marginBottom: 8 }}>Add upgrades</div>
                  <div style={{ display: "grid", gap: 8, fontSize: 13 }}>
                    {([
                      ["appliances", "Update appliances"],
                      ["lighting", "Update light fixtures"],
                      ["backsplash", "Update backsplash"],
                      ["countertop", "Update countertops"],
                      ["hardware", "Update cabinet hardware"],
                      ["landscaping", "Freshen landscaping"],
                  ...
                  <div style={{ marginTop: 10, opacity: 0.75, fontSize: 12 }}>
                    Turning these on adds more choices (most apply in the final render).
                  </div>
                </div>

                <div style={{ marginTop: 14 }}>
                  <div style={{ fontWeight: 800, marginBottom: 8 }}>Anything else?</div>
                  <textarea
                    value={userPrompt}
                    onChange={(e) => setUserPrompt(e.target.value)}
                    placeholder="Optional: small custom requests (e.g., 'add a modern black faucet', 'keep counters the same', 'make it brighter')."
                    style={{
                      width: "100%",
                      minHeight: 90,
                      resize: "vertical",
