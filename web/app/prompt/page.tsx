...
export default function PromptPage() {
  ...
  async function generatePromptedAndContinue() {
    ...
    try {
      ...
      // If overlays are actively building, give them a short head start so the build page feels instant.
      // We only wait in the "building" state; if it's "idle" or "error", proceed immediately.
      if (overlayStatusRef.current === "building") {
        setLoadingMsg(
          `Preparing instant options… ${Math.round((overlayProgressRef.current || 0) * 100)}%`
        );
        const start = Date.now();
        const maxWaitMs = 8000;
        while (overlayStatusRef.current === "building" && Date.now() - start < maxWaitMs) {
