...
/**
 * Cache for refined preview images.
 * `key` should already be a stable identifier, e.g.
 * `${scanId}:${hash(selections+prompt+extras)}:low`.
 */
export async function getRefine(key: string): Promise<string | null> {
  const out = await redisPipeline([["GET", refineKey(key)]]);
  const raw = out?.[0]?.result;
  if (!raw || typeof raw !== "string") return null;
  return raw;
}

export async function setRefine(key: string, base64Png: string, ttlSeconds = 60 * 60) {
  await redisPipeline([["SET", refineKey(key), base64Png, "EX", String(ttlSeconds)]]);
}
