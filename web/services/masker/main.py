...
# ------------------------------------------------------------
# Coarse localization (BEFORE SAM2)
# ------------------------------------------------------------
#
# We intentionally DO NOT trust GPT-provided polygons for geometry, because
# they often come back as coarse slabs (e.g., full-width horizontal bands).
#
# Instead, we use an open-vocabulary box detector (GroundingDINO via
# Transformers) to get tight-ish boxes for each surface type, then we use
# SAM2 strictly to refine boundaries inside those boxes.
#
# This makes SAM2 usable for our scenario and prevents constant fallback to
# polygon-raster.
...
def _load_grounding_dino():
    """Lazy-load GroundingDINO (open-vocabulary box detector)."""
    ...
    try:
        import torch  # noqa
        from transformers import AutoModelForZeroShotObjectDetection, AutoProcessor  # noqa
    except Exception as e:
        raise RuntimeError(f"GroundingDINO import failed (missing deps?): {e}")

    model_id = os.environ.get("GDINO_MODEL_ID", "IDEA-Research/grounding-dino-base").strip() or "IDEA-Research/grounding-dino-base"
    processor = AutoProcessor.from_pretrained(model_id)
    model = AutoModelForZeroShotObjectDetection.from_pretrained(model_id)
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model.to(device)
    model.eval()
    _GDINO_MODEL, _GDINO_PROCESSOR = model, processor
    return _GDINO_MODEL, _GDINO_PROCESSOR


_TYPE_CAPTIONS: Dict[str, str] = {
    "floor": "floor. flooring. tile floor. wood floor.",
    "walls": "wall. walls.",
    "ceiling": "ceiling.",
    "cabinets": "kitchen cabinets. cabinetry. cabinet doors.",
    "countertop": "countertop. counter top. kitchen counter.",
    "backsplash": "backsplash. tile backsplash.",
    "appliances": "refrigerator. oven. stove. dishwasher. microwave.",
...
def _choose_boxes_for_type(img_rgb: np.ndarray, item_type: str) -> List[Tuple[int, int, int, int]]:
    """Return 1-3 tight-ish boxes for the given type using open-vocab detection."""
    model, processor = _load_grounding_dino()
    caption = _TYPE_CAPTIONS.get(item_type, item_type)

    # thresholds can be tuned via env without code changes
    box_thresh = float(os.environ.get("GDINO_BOX_THRESHOLD", "0.25"))
    text_thresh = float(os.environ.get("GDINO_TEXT_THRESHOLD", "0.20"))

    import torch

    inputs = processor(images=img_rgb, text=caption, return_tensors="pt")
    inputs = {k: v.to(model.device) for k, v in inputs.items()}
    with torch.no_grad():
        outputs = model(**inputs)

    # Post-process to absolute pixel boxes
    target_sizes = torch.tensor([img_rgb.shape[:2]], device=model.device)
    ...
    # Sort by score*area (favor confident large regions)
    ...
    for b, s in zip(boxes_np, scores_np):
        ...
        scored.append((float(s) * (area ** 0.5), (x0, y0, x1, y1)))

    if not scored:
        return []
    scored.sort(key=lambda x: x[0], reverse=True)

    # Some categories benefit from multiple boxes (appliances/cabinets)
    max_boxes = 3 if item_type in ("appliances", "cabinets", "windows") else 1
    out: List[Tuple[int, int, int, int]] = []
    for _, b in scored:
        # simple NMS-like suppression
        keep = True
        for ob in out:
            ox0, oy0, ox1, oy1 = ob
            ix0 = max(b[0], ox0)
            ...
            union = (b[2] - b[0]) * (b[3] - b[1]) + (ox1 - ox0) * (oy1 - oy0) - inter
...
def sam2_mask(img_bgr: np.ndarray, polys_norm: List[List[Tuple[float, float]]], item_type: str) -> np.ndarray:
    ...
    # Convert polygon hints (if present) to a coarse seed.
    polys_px = polygons_to_px(polys_norm, w, h)
    bbox0 = polygons_to_bbox(polys_px, w, h)
    seed_poly = union_polygon_mask(h, w, polys_px).astype(np.uint8) if bbox0 is not None else np.zeros((h, w), dtype=np.uint8)

    # We decide whether the polygon geometry is "trustworthy". GPT polygons are often slabs.
    # If they are too large / band-like, we ignore them and use open-vocab detection instead.
    poly_is_slab = False
    if bbox0 is None or seed_poly.sum() < 50:
        poly_is_slab = True
    else:
        x0, y0, x1, y1 = bbox0
        bbox_area0 = max(1, (x1 - x0) * (y1 - y0))
        img_area = h * w
        fill_ratio = float(seed_poly.sum()) / float(bbox_area0)
        bbox_ratio = float(bbox_area0) / float(img_area)
        # slab heuristics: huge bbox or nearly-rectangular fill
        if bbox_ratio > 0.60 or fill_ratio > 0.97:
            poly_is_slab = True
    ...
    def _run_sam_on_box(box_xyxy: Tuple[int, int, int, int], seed01: np.ndarray) -> np.ndarray:
        """Run SAM2 in a tight box with structured pos/neg points and return a 0/1 mask."""
        bx0, by0, bx1, by1 = box_xyxy
        pad = int(max(8, 0.03 * max(bx1 - bx0, by1 - by0)))
        bbox = _expand_bbox((bx0, by0, bx1, by1), pad)
        bx0, by0, bx1, by1 = bbox

        # Positive points from eroded seed (more interior, less edge ambiguity)
        inner = cv2.erode(seed01.astype(np.uint8) * 255, kernel, iterations=1)
        inner01 = (inner > 0).astype(np.uint8)
        pos = np.concatenate([_center_point(inner01), _sample_points(inner01, 6)], axis=0)
        if pos.shape[0] == 0:
            pos = _center_point(seed01)

        # Negative points: ring just outside the seed (within expanded bbox)
        dil = cv2.dilate(seed01.astype(np.uint8) * 255, kernel, iterations=2)
        ring = ((dil > 0) & (seed01 == 0)).astype(np.uint8)
        ...
        # Choose best by IoU with the seed + SAM score, penalize flooding.
        seed_area = float(seed01.sum())
        best_idx = 0
        best_val = -1e9
        ...
        for i in range(len(masks)):
            ...
            val = (0.70 * iou) + (0.30 * float(scores[i])) - (0.30 * area_pen)
            if seed_area < 100:
                # if seed is tiny, rely more on score and less on IoU
                val = (0.50 * float(scores[i])) - (0.25 * area_pen)
            if val > best_val:
                best_val = val
                best_idx = i
        ...
        # Tighten and run a second pass (improves edge snap)
        ys, xs = np.where(m > 0)
        if xs.size > 50:
        ...
        # Final sanity
        if not _sanity(m, (bx0, by0, bx1, by1)):
            return np.zeros((h, w), dtype=np.uint8)
        return m

    # Choose coarse boxes.
    boxes: List[Tuple[int, int, int, int]] = []
    seed_for_boxes: List[np.ndarray] = []

    if not poly_is_slab and bbox0 is not None:
        # Use polygon bbox and polygon seed
        boxes = [bbox0]
        seed_for_boxes = [seed_poly]
    else:
        # Use open-vocab detection to get tight-ish boxes. This is the key fix.
        try:
            boxes = _choose_boxes_for_type(img_rgb, item_type)
        except Exception:
            boxes = []
        if boxes:
            for (x0, y0, x1, y1) in boxes:
                seed = np.zeros((h, w), dtype=np.uint8)
                cv2.rectangle(seed, (x0, y0), (x1, y1), 1, thickness=-1)
                seed_for_boxes.append(seed)
        else:
            # No detector box available; fall back to polygon if we have it
            if bbox0 is None:
                return np.zeros((h, w), dtype=np.uint8)
            boxes = [bbox0]
            seed_for_boxes = [seed_poly]

    # Run SAM2 per box and union results (useful for cabinets/appliances)
    merged = np.zeros((h, w), dtype=np.uint8)
    for b, s in zip(boxes, seed_for_boxes):
        m = _run_sam_on_box(b, s)
        if m.sum() > 0:
            merged = np.where((merged == 1) | (m == 1), 1, 0).astype(np.uint8)
...
def preview_masks(
    ...
    for item in req.items:
        ...
        try:
            if method == "sam2":
                m = sam2_mask(img, item.polygons_norm, item.type)
                used = "sam2"
