...
export default function BuildPage() {
  const { s, go, setSelections, setMaskOverrides, rebuildOverlays, refineMasks } = useWizard();
  ...
  const drawOverlays = useCallback(async () => {
    ...
    for (const { m, selected } of selectedModules) {
      ...
      // Next-best: refined auto masks (pixel-level) if available.
      const auto = s.autoMasks?.byType || {};
      const autoSources = (m.targetElementTypes || [])
        .map((t) => auto?.[t])
        .filter((src): src is string => typeof src === "string" && src.length > 0);

      if (autoSources.length) {
        const key = `auto:${m.featureId}:${Math.round(rect.width)}x${Math.round(rect.height)}`;
        let autoMask = polyMaskCache.current.get(key);
        if (!autoMask) {
          ...
          if (mctx) {
            ...
            // Union by drawing each transparent PNG mask.
            for (const src of autoSources) {
              const mimg = await ensureImage(src, imgCache.current);
              mctx.drawImage(mimg, 0, 0, maskCanvas.width, maskCanvas.height);
            }
          }
          autoMask = maskCanvas;
          polyMaskCache.current.set(key, autoMask);
        }

        ctx.globalCompositeOperation = "destination-in";
        ctx.globalAlpha = 1;
        ctx.drawImage(autoMask, 0, 0, rect.width, rect.height);
        ctx.globalCompositeOperation = "source-over";
        continue;
      }
      ...
      if (!polyMask) {
        ...
        if (mctx) {
          ...
          for (const el of els) {
            ...
            mctx.beginPath();
            // points_norm is a tuple: [xNorm, yNorm]
            mctx.moveTo(pts[0][0] * maskCanvas.width, pts[0][1] * maskCanvas.height);
            for (let i = 1; i < pts.length; i++) {
              mctx.lineTo(pts[i][0] * maskCanvas.width, pts[i][1] * maskCanvas.height);
            }
            mctx.closePath();
            mctx.fill();
          }
  ...
  }, [activeAfterSrc, precomputedReady, s.sceneGraph, s.overlayAssets, s.overlayStatus, s.maskOverrides, s.autoMasks, selectedModules]);
  ...
  const onSelect = useCallback(
    (featureId: string, optionId: string) => {
      ...
      // On-demand premium masking: when the user interacts with a category, request
      // pixel-accurate masks for that module's target surfaces, then rebuild only that overlay.
      const mod = (s.modules || []).find((m) => m.featureId === featureId);
      const types = mod?.targetElementTypes || [];
      if (types.length) {
        void (async () => {
          try {
            await refineMasks(types);
            await rebuildOverlays(featureId);
          } catch {
            // If refinement fails, the UI will fall back to polygon masks.
          }
        })();
      }
    },
    [activeAfterSrc, selections, setSelections, s.modules, refineMasks, rebuildOverlays]
  );
  ...
  const openMaskEditor = useCallback(
      ...
      try {
        ...
        if (existing) {
          ...
          if (cached) {
            ...
            // Best: union refined auto masks (pixel-level) if available.
            // Fallback: union scan polygons.
            ...
            const auto = s.autoMasks?.byType || {};
            const autoSources = Array.from(types)
              .map((t) => auto?.[t])
              .filter((src): src is string => typeof src === "string" && src.length > 0);
        ...
        const firstType = (m.targetElementTypes?.[0] || "walls") as SceneElementType;
        setMaskElement({
          id: m.featureId,
          type: firstType,
          label: m.label,
    ...
    [beforeSrc, s.sceneGraph, s.maskOverrides, s.autoMasks]
  );
                    ...
                    <div key={m.featureId} style={{ padding: 12, borderRadius: 16, border: "1px solid rgba(255,255,255,0.10)", background: "rgba(255,255,255,0.03)" }}>
                      ...
                      <div style={{ marginTop: 10, display: "flex", gap: 10, flexWrap: "wrap" }}>
                        {m.options.slice(0, 3).map((opt) => {
                          ...
                          const isColor = preview?.kind === "color";
