...
export default function PromptPage() {
  const { s, go, setUserPrompt, setExtras, setPromptedSrc, setSelections } = useWizard();
  ...
  async function generatePromptedAndContinue() {
    ...
    try {
      await go("/build", {
        message: "Generating your baseline remodel…",
        submessage: "We’ll use your prompt, then you can mix & match options instantly.",
        minMs: 3000,
        before: async () => {
          ...
          // Enter the builder with no preselected options.
          setSelections({});
        },
      });
    } catch (e: any) {
      ...
      <div className="kv-card kv-surface kv-app-card" style={{ width: "min(900px, 100%)", padding: 20 }}>
        ...
        <div style={{ marginTop: 16, display: "flex", gap: 12, justifyContent: "space-between", flexWrap: "wrap" }}>
          <button
            ...
            onClick={() => void go("/contact", { message: "Returning…", minMs: 3000 })}
            disabled={busy}
          >
            Back
          </button>
          <button className="kv-btn" onClick={generatePromptedAndContinue} disabled={!canNext || busy}>
            {busy ? "Working…" : "Build my remodel"}
          </button>
        </div>
      </div>
    </div>
