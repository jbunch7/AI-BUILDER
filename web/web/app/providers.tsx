...
import type { BuilderModule, SceneElementType, SceneGraph } from "@/lib/builder/types";
...
export type AutoMasks = {
  ...
  width: number;
  height: number;
  byType: Partial<Record<SceneElementType, string>>;
  source?: string;
...
export type WizardSession = {
  ...
  /** Pixel masks (PNG data URLs) refined from scanner polygons for precise previews. */
  autoMasks: AutoMasks;
...
type Ctx = {
  ...
  /** Ask the server to generate premium, pixel-accurate masks for the given element types (on-demand). */
  refineMasks: (types: SceneElementType[]) => Promise<void>;
...
// Hook used by all step pages (upload/contact/prompt/build).
...
function toPersistedSession(s: WizardSession) {
  ...
  const { beforeSrc: _b, promptedSrc: _p, overlayAssets: _o, maskOverrides: _m, autoMasks: _am, ...rest } = s;
  return rest;
}
...
async function buildOverlayAssetsClient(args: {
  ...
  const { baseSrc, scene, modules, maskOverrides, autoMasks, maxSide = PREVIEW_MAX_SIDE, onProgress } = args;
  ...
  const buildMaskCanvas = async (m: BuilderModule) => {
    ...
    // Next-best: refined auto masks (pixel masks), generated during /api/scan.
    const auto = autoMasks?.byType || {};
    const autoSources = (m.targetElementTypes || [])
      .map((t) => auto?.[t])
    ...
    if (autoSources.length) {
      ...
      if (!mctx) return null;
      mctx.clearRect(0, 0, w, h);
      // Union all masks by drawing them; transparency outside the mask keeps it clean.
      for (const src of autoSources) {
        const mimg = await loadImg(src);
        mctx.drawImage(mimg, 0, 0, w, h);
      }
      return mask;
    }

    // Fallback: rasterize scanner polygons.
...
type TransitionState = {
  // Optional so callers can set partial state safely.
  // The overlay will provide a default message.
  message?: string;
  submessage?: string;
};
    ...
    >
      ...
      >
        <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 900, fontSize: 16, letterSpacing: -0.2 }}>{state.message ?? "Loading…"}</div>
...
export function WizardProvider({ children }: { children: React.ReactNode }) {
  ...
  const refineMasks = useCallback(
    async (types: SceneElementType[]) => {
      if (s.scanStatus !== "ready") return;
      if (!s.scanId) return;
      if (!types?.length) return;

      // Deduplicate and ignore types we already have *premium* masks for.
      // IMPORTANT: When auto masks are generated from polygon rasterization during scan,
      // we still want to refine them (call the mask service) even though a mask string exists.
      const want = Array.from(new Set(types)).filter((t) => {
        const existing = s.autoMasks?.byType?.[t];
        const hasMask = typeof existing === "string" && existing.length > 0;

        // If masks already came from the mask service, we consider them premium.
        const alreadyPremium = s.autoMasks?.source === "mask-service";

        // Only skip if we already have a premium mask for this type.
        return !(hasMask && alreadyPremium);
      });

      if (!want.length) return;

      const res = await fetch("/api/mask/refine", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ scanId: s.scanId, types: want }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data?.error || data?.message || "Mask refine failed");

      const incoming = data?.autoMasks?.byType || {};
      setS((p) => {
        const merged = {
          maxSide: data?.autoMasks?.maxSide ?? p.autoMasks?.maxSide ?? PREVIEW_MAX_SIDE,
          width: data?.autoMasks?.width ?? p.autoMasks?.width ?? 0,
          height: data?.autoMasks?.height ?? p.autoMasks?.height ?? 0,
          byType: { ...(p.autoMasks?.byType || {}), ...(incoming as any) },
          source: data?.autoMasks?.source ?? p.autoMasks?.source,
          errors: { ...(p.autoMasks?.errors || {}), ...(data?.autoMasks?.errors || {}) },
        };
        return {
          ...p,
          autoMasks: merged,
          // Refined masks change overlays; rebuild lazily.
          overlayAssets: {},
          overlayStatus: "idle",
          overlayProgress: 0,
          overlayError: undefined,
    ...
    [s.scanStatus, s.scanId, s.autoMasks]
  );
  ...
  const rebuildOverlays = useCallback(
    ...
    [s.overlayStatus, s.scanStatus, s.beforeSrc, s.sceneGraph, s.modules, s.maskOverrides, s.autoMasks]
  );
  ...
  const startScan = useCallback(async (file: File) => {
    ...
    const run = (async () => {
      ...
      try {
        ...
        const autoMasks = (data.autoMasks ?? null) as AutoMasks;
  ...
  const value = useMemo<Ctx>(
    ...
    [s, resetAll, setBeforeSrc, setPromptedSrc, startScan, setUserPrompt, setExtras, setSelections, setMaskOverrides, refineMasks, rebuildOverlays, go]
  );
