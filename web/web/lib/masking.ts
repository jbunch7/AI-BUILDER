...
function uniqueTypesFromScene(sceneGraph: SceneGraph): SceneElementType[] {
  const out: SceneElementType[] = [];
...
async function callMaskService(args: {
  ...
  // If polygons are empty (we don't use LLM geometry), we still send requested types.
  const types = uniqueTypesFromScene(args.sceneGraph);
  if (!types.length) {
    return { maxSide: args.maxSide, width: 1, height: 1, byType: {}, source: "mask-service" };
  }
  ...
  const payload = {
    ...
    scene: {
      category: (args.sceneGraph as any)?.meta?.category ?? null,
      subcategory: (args.sceneGraph as any)?.meta?.subcategory ?? null,
    },
    items: types.map((t) => ({
      type: t,
  ...
  const timeoutMs = Number(process.env.MASK_SERVICE_TIMEOUT_MS || 45000);
...
export async function generateAutoMasks(args: {
  ...
  // IMPORTANT:
  // Our product objective is to have masks ready by the time the user reaches the editor.
  // Therefore, if a mask service is configured, we call it during scan by default.
  // You may disable this behavior (for cost-testing) by setting MASK_AUTO_ON_SCAN=0.
  const autoOnScan = process.env.MASK_AUTO_ON_SCAN !== "0";
  ...
  if (autoOnScan && process.env.MASK_SERVICE_URL) {
    try {
      ...
      // fall through to polygon raster fallback
    }
  }
