...
async function callMaskService(args: {
  ...
  // Modal cold-start + model load can exceed 12s on first request.
  // Use a safer default; can still be overridden via env.
  const timeoutMs = Number(process.env.MASK_SERVICE_TIMEOUT_MS || 60000);
...
export async function generateAutoMasks(args: {
  ...
  // UX GOAL:
  // By default we DO call the mask service during scan so masks are ready
  // by the time the user reaches the editor.
  // Set MASK_AUTO_ON_SCAN=0 to disable this (cost-control mode).
