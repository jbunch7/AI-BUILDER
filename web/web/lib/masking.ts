...
export async function generateAutoMasks(args: {
  ...
  // DEFAULT BEHAVIOR (UX PRIORITY):
  // If a mask service is configured, generate premium masks during scan so they're ready
  // by the time the user reaches the builder.
  //
  // COST CONTROL:
  // Set MASK_AUTO_ON_SCAN=0 to disable premium masking on scan and fall back to polygon raster.
