...
def sam2_mask(img_bgr: np.ndarray, polys_norm: List[List[Tuple[float, float]]]) -> np.ndarray:
    """Use SAM2 to predict a binary mask from a polygon-derived box prompt."""
    h, w = img_bgr.shape[:2]
    polys_px = polygons_to_px(polys_norm, w, h)
    bbox = polygons_to_bbox(polys_px, w, h)
    if bbox is None:
    ...
    # (Optional) intersect with polygon union to avoid gross overreach
    poly_union = union_polygon_mask(h, w, polys_px)
    if poly_union.sum() > 0:
        m = (m & poly_union).astype(np.uint8)
    return m
...
def preview_masks(
    ...
    for t, m in resolved.items():
        out[t] = MaskOut(png_base64=to_png_base64(m), confidence=1.0, method="sam2" if req.method=="sam2" else "mixed")
